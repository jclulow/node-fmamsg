...
/*
 * Initialise the CRC table as BigIntegers using the
 * strings table...
 */
var CRC_TABLE = [];
for (i = 0; i < CRC_TABLE_STR.length; i++) {
	CRC_TABLE.push(new bigdecimal.BigInteger(CRC_TABLE_STR[i], 16));
}
CRC_TABLE_STR = null; /* don't need this anymore */

/* we use 0xFF a few times ... */
var CRC_FF = new bigdecimal.BigInteger('ff', 16);

module.exports.crc = function crc(cval, val) {
	if (typeof (val) === 'number')
		val = bigdecimal.BigInteger.valueOf(val);
	var a = cval.shiftLeft(8);
	var idx = cval.shiftRight(24).and(CRC_FF).xor(val);
	var b = CRC_TABLE[idx.and(CRC_FF).intValue()];
	return (a.xor(b));
}
